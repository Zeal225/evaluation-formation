                ...
                <div class="card-body">
                    <form action="{{ route("modifier_formation_ok", ["id"=>$formation->id_formation]) }}" method="post">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-9">
                                <input name="theme_formation" value="{{ old("theme_formation") ?? $formation->theme_formation }}" class="form-control" id="theme_formation" placeholder="Entrez le thème de la formation">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-9">
                                <input name="lieu_formation" value="{{ old("lieu_formation") ?? $formation_utilisateur->lieu_formation }}" class="form-control" id="lieu_formation" placeholder="Entrez le lieu de la formation">
                            ...
                            <div class="col-lg-9">
                                <select name="id_formateur" id="id_formateur" class="form-select" aria-label="Default select example">
                                    ...
                                    @foreach($formateurs as $formateur)
                                        <option @if($formateur->id == old("id_formateur") || $formation->id_formateur == $formateur->id) selected @endif value="{{ $formateur->id }}">{{ $formateur->nom_formateur }} {{ $formateur->prenom_formateur }} - {{ $formateur->specialite_formateur }} - {{ $formateur->cabinet->nom_cabinet }}</option>
                                    @endforeach
                                </select>
                                @error('id_formateur')
                                <span class="text-danger">
                            ...
                            <div class="col-lg-9">
                                <select class="form-control" id="id_utilisateur" data-choices data-choices-removeItem name="id_utilisateur[]" multiple>
                                    ...
                                    @foreach($utilisateurs as $utilisateur)
                                        <option @if(in_array($utilisateur->id, old("id_utilisateur", [])) || $participants->contains("id_utilisateur", "=", $utilisateur->id)) selected @endif value="{{ $utilisateur->id }}">{{ $utilisateur->nom_utilisateur }} {{ $utilisateur->prenom_utilisateur }} </option>
                                    @endforeach
                                </select>
